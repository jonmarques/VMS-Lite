...
public class OnvifDiscoveryService {
				...
				while (true) {
					try {
						...
						if (!dispositivosEncontrados.containsKey(deviceIp)) {
							...
							if (uuid != null) {
								System.out.println("UUID do dispositivo " + deviceIp + ": " + uuid);
							} else {
								System.out.println("Aviso: dispositivo " + deviceIp + " não retornou UUID no ProbeMatch.");
							}
	...
	private static String extrairUuid(String xml) {
	    try {
	        // Ancora na tag Address (com ou sem prefixo de namespace), aceitando
	        // tanto "urn:uuid:" quanto apenas "uuid:" (variação usada por TP-Link/Tapo/VIGI)
	...
	/**
	 * Procura, dentre os dispositivos descobertos via WS-Discovery, aquele cujo UUID
	 * corresponde ao UUID salvo. Retorna o IP correspondente, ou null se não encontrado.
	 */
	...
	/**
	 * Substitui o endereço IP dentro de uma URL RTSP pelo novo IP informado, trocando
	 * exatamente o trecho encontrado pelo regex (evita ambiguidade de substring que
	 * String.replace() poderia causar em casos raros).
	 * Retorna a URL original inalterada se nenhum IP for encontrado nela.
	 */
	...
	/**
	 * Extrai o primeiro endereço IP encontrado na URL, ou null se não houver.
	 */
